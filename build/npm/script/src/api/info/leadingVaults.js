...
exports.LeadingVaultsResponse = (() => {
    ...
    v.pipe(v.object({
        /** Vault address. */
        address: v.pipe(_base_js_1.Address, v.description("Vault address.")),
        /** Vault name. */
        name: v.pipe(v.string(), v.description("Vault name.")),
    }), v.description("Vault that a user is leading."))), v.description("Array of leading vaults for a user."));
})();
/**
 * Request leading vaults for a user.
...
function leadingVaults(config, params, signal) {
    const request = (0, _base_js_1.parser)(exports.LeadingVaultsRequest)({
        type: "leadingVaults",
        ...params,
    });
